...
class MultiImgPackSegInputs(BaseTransform):
    ...
    def __init__(self,
                 meta_keys=('img_path', 'seg_map_path', 'seg_map_path_from', 
                            'seg_map_path_to','gaussian_map_path', 'ori_shape','img_shape', 
    ...
    def transform(self, results: dict) -> dict:
        ...
        if 'gt_gaussian' in results:
            gt_gaussian_data = dict(
                data=to_tensor(results['gt_gaussian'][None,
                                                     ...].astype(np.float32)))
            data_sample.set_data(dict(gt_gaussian=PixelData(**gt_gaussian_data)))
        img_meta = {}
