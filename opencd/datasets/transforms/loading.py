...
@TRANSFORMS.register_module()
class MultiImgMultiAnnLoadAnnotations_Gaussian(MMCV_LoadAnnotations):
    """Load annotations for semantic change detection provided by dataset.
    ...
    def _load_seg_map(self, results: dict) -> None:
        ...
        gt_semantic_seg = mmcv.imfrombytes(
            img_bytes, flag='grayscale', # in mmseg: unchanged
            backend=self.imdecode_backend).squeeze().astype(np.uint8)
        img_bytes_gaussian = fileio.get(
            results['gaussian_map_path'], backend_args=self.backend_args)
        gt_gaussian = mmcv.imfrombytes(
            img_bytes_gaussian, flag='unchanged',
            backend='tifffile').squeeze().astype(np.float32)
        ...
        results['gt_seg_map'] = gt_semantic_seg
        # print(gt_gaussian.max())
        results['gt_gaussian'] = gt_gaussian
        results['seg_fields'].extend(['gt_seg_map', 
            'gt_gaussian'])
